# =========================================================================
# HYPRLAND + VIRT-MANAGER + RICE STACK + TERMINAL TOOLS + LIBREOFFICE (V14)
# =========================================================================
{ config, pkgs, ... }:
{
  imports = [
  ...
  virtualisation.libvirtd = {
    enable = true;
    onBoot = "start"; # Auto-starts hypervisor daemon on boot
    qemu = {
      package = pkgs.qemu_kvm;
      runAsRoot = true;
    };
    extraConfig = ''
      network_autostart = ["default"]
    '';
  };
  virtualisation.spiceUSBRedirection.enable = true; # Pass-through USB devices to VMs
  ...
  environment.systemPackages = with pkgs; [
    # Office Suite
    libreoffice-fresh    # Restores working LibreOffice

    # CLI Terminal Suite (TikTok Rice Tools)
    yazi                 # Terminal TUI File Manager with Image Preview
    fzf                  # Fuzzy Finder
    fd                   # Fast directory search replacement for find
    ripgrep              # Fast text content search (rg)
    ...
    qemu_kvm
    OVMF
    spice-gtk            # For USB redirection support in virt-manager UI
    dnsmasq              # Handles NAT DHCP for virt-manager's default network
    bridge-utils         # Manages virtual bridge devices (virbr0)
